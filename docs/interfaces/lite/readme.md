...
The Cosmos SDK Light Client (Infinitefuture-lite) is split into two separate components. The first component is generic for
...
API as any Tendermint Core node. The second component is specific for the Cosmos Hub (`infinitefutured`). It works as a query
...
All applications are expected to run only against Infinitefuture-lite. Infinitefuture-lite is the only piece of software
...
|| Full Node | Infinitefuture-lite | Description|
|-| ------------- | ----- | -------------- |
| Execute and verify transactions|Yes|No|A full node will execute and verify all transactions while Infinitefuture-lite won't.|
| Verify and save blocks|Yes|No|A full node will verify and save all blocks while Infinitefuture-lite won't.|
...
| Storage resources|High|Low|A full node will save all blocks and ABCI states. Infinitefuture-lite just saves validator sets and some checkpoints.|
| Power consumption|High|Low|Full nodes must be deployed on machines which have high performance and will be running all the time. Infinitefuture-lite can be deployed on the same machines as users' applications, or on independent machines but with lower performance. Light clients can be shut down anytime when necessary. Infinitefuture-lite consumes very little power, so even mobile devices can meet the power requirements.|
| Provide APIs|All cosmos APIs|Modular APIs|A full node supports all Cosmos APIs. Infinitefuture-lite provides modular APIs according to users' configuration.|
...
According to the above table, Infinitefuture-lite can meet many users' functionality and security requirements, but require little bandwidth, computing, storage, and power.

## Achieving Security

### Trusted Validator Set

The base design philosophy of Infinitefuture-lite follows two rules:
...
validator set comes from a genesis file. During runtime, if Infinitefuture-lite detects a different validator set,

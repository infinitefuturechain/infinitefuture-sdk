...
```bash
infinitefuturecli keys add <your_key_name>
```
...
```bash
infinitefuturecli account <YOUR_ADDRESS>
```
...
```bash
infinitefuturecli tx send <from_key_or_address> <to_address> <amount> \
...
```bash
infinitefuturecli rest-server --node=<full_node_address:full_node_port>
```
...
每个 Cosmos SDK 交易都有一个规范的 JSON 描述。 `infinitefuturecli`和 REST 接口为交易提供规范的 JSON 描述，“广播”功能将提供紧凑的 Amino（类似 protobuf 的格式）编码转换。

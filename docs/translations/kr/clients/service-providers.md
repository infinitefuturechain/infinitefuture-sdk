...
```bash
infinitefuturecli send --amount=10faucetToken --chain-id=<name_of_testnet_chain> --from=<key_name> --to=<destination_address>
```
...
```bash
infinitefuturecli advanced rest-server --node=<full_node_address:full_node_port>
```
